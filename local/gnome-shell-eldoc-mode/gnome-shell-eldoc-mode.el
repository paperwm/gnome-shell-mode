;;; gnome-shell-eldoc-mode.el --- Display the arguments for the standard Lua functions in the minibuffer  -*- lexical-binding: t; -*-
...
;;
;;     (require 'gnome-shell-eldoc-mode)
;;     (add-hook 'lua-mode-hook 'gnome-shell-eldoc-mode)
;; 
...
(defvar gnome-shell-eldoc-mode-standard-functions
  '(
...
(defun gnome-shell-eldoc-mode-thing-at-point-no-properties (thing)
...
(defun gnome-shell-eldoc-mode--doc-for-stdlib (function-name)
  ...
  (if (s-contains? ":" function-name)
      (cdr (assoc (-last-item (s-split ":" function-name)) gnome-shell-eldoc-mode-standard-functions))
    (cdr (assoc function-name gnome-shell-eldoc-mode-standard-functions)))
  )

(defvar gnome-shell-eldoc-mode--last-lookup '("" . "")
...
(defun gnome-shell-eldoc-mode-help-at-point ()
  ...
  (with-syntax-table (copy-syntax-table)
    ...
    (save-excursion
      (backward-word)
      (let* ((function-name (gnome-shell-eldoc-mode-thing-at-point-no-properties 'symbol)))
        ...
        (when (not (null function-name))
          (if (equal (car gnome-shell-eldoc-mode--last-lookup) function-name)
              (cdr gnome-shell-eldoc-mode--last-lookup)
            (let ((eldoc (or (gnome-shell-eldoc-mode--doc-for-stdlib function-name)
                             (gnome-shell-eldoc function-name))))
              (setcar gnome-shell-eldoc-mode--last-lookup function-name)
              (setcdr gnome-shell-eldoc-mode--last-lookup eldoc)
              eldoc)))))))


;;;###autoload
(defun gnome-shell-eldoc-mode (&optional arg)
  "Display in the minibuffer the arguments that the standard Lua functions take."
  (interactive (list (or current-prefix-arg 'toggle)))
  (set (make-local-variable 'eldoc-documentation-function) 'gnome-shell-eldoc-mode-help-at-point)
  (eldoc-mode arg))


(provide 'gnome-shell-eldoc-mode)

;;; gnome-shell-eldoc-mode.el ends here
